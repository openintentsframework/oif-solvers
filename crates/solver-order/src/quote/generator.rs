...
impl QuoteGenerator {
	pub fn new() -> Self {
		Self { custody_strategy: CustodyStrategy::new() }
	}
	...
	async fn generate_quote_for_settlement(
		...
		let order = match settlement_decision {
			CustodyDecision::ResourceLock { kind } => self.generate_resource_lock_order(request, config, kind)?,
			CustodyDecision::Escrow { kind } => self.generate_escrow_order(request, config, kind)?,
		};
		let details = QuoteDetails { requested_outputs: request.requested_outputs.clone(), available_inputs: request.available_inputs.clone() };
		let eta = self.calculate_eta(&request.preference);
		Ok(Quote { orders: vec![order], details, valid_until: Some(chrono::Utc::now().timestamp() as u64 + 300), eta: Some(eta), quote_id, provider: "oif-solver".to_string() })
	}
	...
	fn generate_resource_lock_order(
		...
		let (primary_type, message) = match lock_kind {
			LockKind::TheCompact { params } => ("CompactLock".to_string(), self.build_compact_message(request, params)?),
		};
		Ok(QuoteOrder { signature_type: SignatureType::Eip712, domain: domain_address, primary_type, message })
	}
	...
	fn generate_permit2_order(
		...
		let chain_id = request.available_inputs[0].asset.ethereum_chain_id().map_err(|e| QuoteError::InvalidRequest(format!("Invalid chain ID in asset address: {}", e)))?;
		let domain_address = permit2_payload::permit2_domain_address_from_config(config, chain_id)?;
		let (final_digest, message_obj) = permit2_payload::build_permit2_batch_witness_digest(request, config)?;
		let message = serde_json::json!({ "digest": with_0x_prefix(&hex::encode(final_digest)), "eip712": message_obj });
		Ok(QuoteOrder { signature_type: SignatureType::Eip712, domain: domain_address, primary_type: "PermitBatchWitnessTransferFrom".to_string(), message })
	}
	...
	fn generate_erc3009_order(
		...
		Ok(QuoteOrder { signature_type: SignatureType::Erc3009, domain: domain_address, primary_type: "ReceiveWithAuthorization".to_string(), message })
	}

	fn build_compact_message(&self, request: &GetQuoteRequest, _params: &serde_json::Value) -> Result<serde_json::Value, QuoteError> {
		Ok(serde_json::json!({
			"user": request.user,
	...
	fn get_lock_domain_address(&self, config: &Config, lock_kind: &LockKind) -> Result<InteropAddress, QuoteError> {
		match &config.settlement.domain {
			Some(domain_config) => {
				let address = domain_config.address.parse().map_err(|e| QuoteError::InvalidRequest(format!("Invalid domain address in config: {}", e)))?;
				Ok(InteropAddress::new_ethereum(domain_config.chain_id, address))
			}
			None => Err(QuoteError::InvalidRequest(format!("Domain configuration required for lock type: {:?}", lock_kind))),
		}
	}

	fn get_escrow_address(&self, config: &Config) -> Result<alloy_primitives::Address, QuoteError> {
		match &config.settlement.domain {
			Some(domain_config) => domain_config.address.parse().map_err(|e| QuoteError::InvalidRequest(format!("Invalid escrow address in config: {}", e))),
			None => Err(QuoteError::InvalidRequest("Escrow address configuration required".to_string())),
		}
	}
	...
	fn sort_quotes_by_preference(&self, quotes: &mut [Quote], preference: &Option<QuotePreference>) {
		match preference {
			Some(QuotePreference::Speed) => quotes.sort_by(|a, b| match (a.eta, b.eta) { (Some(eta_a), Some(eta_b)) => eta_a.cmp(&eta_b), (Some(_), None) => std::cmp::Ordering::Less, (None, Some(_)) => std::cmp::Ordering::Greater, (None, None) => std::cmp::Ordering::Equal }),
