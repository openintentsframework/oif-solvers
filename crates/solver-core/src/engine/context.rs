...
impl ContextBuilder {
    ...
    pub fn new(delivery: Arc<DeliveryService>, config: Config) -> Self {
    ...
    fn extract_eip7683_chains(&self, data: &serde_json::Value) -> Result<Vec<u64>, SolverError> {
                ...
                serde_json::Value::String(s) => {
                    if s.starts_with("0x") {
                        ...
                        match u64::from_str_radix(&s[2..], 16) {
                            Ok(parsed) => {
                                tracing::info!("Parsed hex chain ID '{}' as {}", s, parsed);
                                Some(parsed)
                            }
                            Err(e) => {
        ...
        if let Some(outputs) = data.get("requestedOutputs").and_then(|v| v.as_array()) {
            tracing::info!("Found {} requested outputs", outputs.len());
            for (i, output) in outputs.iter().enumerate() {
                if let Some(asset) = output.get("asset").and_then(|v| v.as_str()) {
                    tracing::info!("Output {}: asset = {}", i, asset);
                    match self.extract_chain_from_interop_address(asset) {
                        Ok(chain_id) => {
                            tracing::info!("Extracted chain_id {} from asset {}", chain_id, asset);
                    ...
                    tracing::info!("Output {} has no asset field", i);
                }
            }
        } else {
            tracing::info!("No requestedOutputs found in intent data");
        }
    ...
    fn extract_chain_from_interop_address(&self, address: &str) -> Result<u64, SolverError> {
        ...
        if let Some(eip155_part) = address.strip_prefix("eip155:") {
            ...
            if let Some(colon_pos) = eip155_part.find(':') {
                ...
                let chain_id = if chain_part.starts_with("0x") {
                    // Parse hex chain ID
                    u64::from_str_radix(&chain_part[2..], 16).map_err(|e| {
                        SolverError::Service(format!("Invalid hex chain ID '{}' in address {}: {}", chain_part, address, e))
                    })?
                } else {
                    // Parse decimal chain ID
                    chain_part.parse::<u64>().map_err(|e| {
                        SolverError::Service(format!("Invalid decimal chain ID '{}' in address {}: {}", chain_part, address, e))
                    })?
                };
                
                tracing::trace!("Successfully parsed chain_id {} from {}", chain_id, chain_part);
                return Ok(chain_id);
            } else {
    ...
    async fn fetch_solver_balances(
        ...
        // Get solver address from config (assuming it's the same across all chains)
        let solver_address = self.get_solver_address().await?;
    ...
    /// Gets the solver's address for balance checking.
    ///
    /// Reads the solver address from the order implementation configuration.
    async fn get_solver_address(&self) -> Result<String, SolverError> {
        // Get solver address from order implementation config
        // Looking for [order.implementations.eip7683.solver_address]
        if let Some(eip7683_config) = self.config.order.implementations.get("eip7683") {
            if let Some(solver_address) = eip7683_config.get("solver_address") {
                if let Some(address_str) = solver_address.as_str() {
        ...
        Err(SolverError::Service(
            "Solver address not found in order.implementations.eip7683.solver_address configuration".to_string(),
        ))
    }
